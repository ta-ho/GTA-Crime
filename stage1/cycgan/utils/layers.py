...
class GraphConvolution(Module):
    ...
    def forward(self, input, adj):
        ...
        support = input.matmul(self.weight) # (128, 256, 512) * (512 * 256) = (128, 256, 256)
        output = adj.matmul(support)        # (128, 256, 256) * (128, 256, 256) = (128, 256, 256)
...
# H_dis(i, j) = -|i-j|/sigma, frame 간의 long-range dependencies를 잡으려고
# 근데 논문이랑 다르게 exp(-|i-j|/e)로 구현되어 있네?
class DistanceAdj(Module):
    ...
    # batch_size=128 (batch 2개), max_seqlen=256 (visual_length)
    def forward(self, batch_size, max_seqlen):
        # To support batch operations
        self.arith = np.arange(max_seqlen).reshape(-1, 1)       # frame idx 생성
        dist = pdist(self.arith, metric='cityblock').astype(np.float32)     # 데이터 포인터들 간의 거리를 제공
        self.dist = torch.from_numpy(squareform(dist)).to('cuda')   # self.dist (256, 256)
