    ...
    def _parse_list(self):
        ...
        if self.test_mode is False:
            if args.datasetname == 'UCF':
                if self.is_normal:
                    self.list = self.list[72:]#ucf 810; sht63; xd 9525
                    # print('normal list')
                    # print(self.list)
                else:
                    self.list = self.list[:72]#ucf 810; sht 63; 9525
                    # print('abnormal list')
                    # print(self.list)
...
class GTADataset(data.Dataset):
    def __init__(self, args, is_normal=True, transform=None, test_mode=False, is_preprocessed=False):
        self.modality = args.modality
        self.is_normal = is_normal
        self.dataset = args.datasetname
        if self.dataset == 'UCF':
            if test_mode:
                exit()
            else:
                self.rgb_list_file = 'list/RTFM/newGTA.list'
