...
class UCF_crime(data.DataLoader):
    def __init__(self, root_dir, modal, mode, num_segments, len_feature, seed=-1, is_normal=None):
        ...
        split_path = os.path.join('list','UCF3_{}.list'.format(self.mode))
        ...
        if self.mode == "Train":
            if is_normal is True:
                self.vid_list = self.vid_list[720:]
...
class GTA(data.DataLoader):
    def __init__(self, root_dir, modal, mode, num_segments, len_feature, seed=-1, is_normal=None):
        ...
        self.num_segments = num_segments
        self.len_feature = len_feature
        split_path = os.path.join('list','newGTA.list')
        ...
        if self.mode == "Train":
            if is_normal is True:
                self.vid_list = self.vid_list[1240:3860]
            elif is_normal is False:
                self.vid_list = self.vid_list[:1240] + self.vid_list[3860:]
            else:
                assert (is_normal == None)
                print("Please sure is_normal=[True/False]")
                self.vid_list=[]
