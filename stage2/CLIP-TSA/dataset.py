...
class Dataset(data.Dataset):
    def __init__(self, args, is_normal=True, transform=None, test_mode=False, normal_only=False):
        ...
        if self.dataset in ['shanghai', "sh"]:
            ...
            if test_mode:
                self.rgb_list_file = f'list/{args.visual.lower()}/gta-{args.visual}-test.list'
                ds = "gta_test"
            else:
                self.rgb_list_file = f'list/{args.visual.lower()}/gta-{args.visual}.list'
                ds = "gta_train"
        ...
        if self.test_mode is False:
            if self.dataset in ["sh", 'shanghai']:
                ...
                if self.is_normal:
                    ...
                    assert len(self.list) == 262
                    # print(self.list)
                else:
                    self.list = self.list[:270]
                    assert len(self.list) == 270
                    # print(self.list)
    ...
    def __getitem__(self, index):
        ...
        if self.test_mode:
            ...
            for feature in features:
                feature = process_feat(feature, 24)  # divide a video into 32 segments
